...
class AddressBook(UserDict):
    ...
    def seach_phone(self, *args):
        result = {}
        for record in self.data.values():
            for phone in record.phones:
                for word in args:
                    match_command = re.search(f'(?i){word}', phone.value)
                    if match_command:
    ...
    def search_name(self, *args):
        ...
        for record in self.data.values():
            for word in args:
                match_command = re.search(f'(?i){word}', record.name.value)
                if match_command:
                    result[record.name.value] = record
        if result:
            return result
...
@input_error
def show_contact(**kwargs):
    search_words = list(kwargs.values())
    search_words.pop(0)
    search_phone = contact_book.seach_phone(*search_words)
    search_name = contact_book.search_name(*search_words)
    result = {**search_name, **search_phone}
    return '\n'.join([f'{name} {", ".join(phone.value for phone in record.phones)} {record.birthday.value.date() if hasattr(record, "birthday") else ""}' for name, record in sorted(result.items())])
